...
const TARGET_CHUNK_SIZE_MB = 14.5; // Target slightly lower for safety margin
...
type LargeFileStrategy = 'sample' | 'split' | 'proceed' | null;
type SampleType = 'random' | 'chrono' | 'even' | null;
...
const Alert: React.FC<AlertProps> = ({ type, children }) => {
  const baseClasses = "p-4 rounded-lg border mt-2 text-sm";
  let specificClasses = "";
  ...
  switch (type) {
    ...
    case 'success':
      ...
      IconComponent = () => (
        <svg className="h-5 w-5 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
      ); break;
    case 'warning':
      specificClasses = "bg-yellow-50 border-yellow-300 text-yellow-800";
      IconComponent = () => (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 3.001-1.742 3.001H4.42c-1.53 0-2.493-1.667-1.743-3.001l5.58-9.92zM10 13a1 1 0 110-2 1 1 0 010 2zm-1.75-5.75a.75.75 0 00-1.5 0v4a.75.75 0 001.5 0v-4z" clipRule="evenodd" /></svg>
      ); break;
    case 'error':
      specificClasses = "bg-red-50 border-red-200 text-red-800";
      IconComponent = () => (
        <svg className="h-5 w-5 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" /></svg>
      ); break;
  }
...
function App() {
  ...
  const [largeFileStrategy, setLargeFileStrategy] = useState<LargeFileStrategy>(null);
  const [sampleType, setSampleType] = useState<SampleType>(null);
  ...
  const resetState = useCallback((keepSelections = false) => {
  ...
  const handleSourceTypeChange = (value: SourceType) => {
    ...
    if (value === 'bluesky') { setAccountSource('username'); setObjectIdSource('text'); }
    else if (value === 'youtube') { setAccountSource('channel'); setObjectIdSource(null); }
    else if (value === 'tiktok') { setAccountSource('author'); setObjectIdSource(null); }
    else if (value === 'telegram') { setAccountSource(null); setObjectIdSource('message_text'); }
    else { setAccountSource(null); setObjectIdSource(null); }
  };

  const handleAccountSourceChange = (value: AccountSource) => {
    setAccountSource(value);
    if (sourceType !== 'bluesky' && sourceType !== 'telegram') { setObjectIdSource(null); }
    if (sourceType === 'telegram' && !objectIdSource) { setObjectIdSource('message_text'); }
    resetState(true);
  };
  ...
  const handleObjectIdSourceChange = (value: ObjectIdSource) => {
    ...
    // useCallback to potentially optimize if passed deeply, though likely not needed here
    const getRequiredHeaders = useCallback((): string[] => {
    ...
    }, [sourceType, accountSource, objectIdSource]); // Dependencies for the function
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    resetState(true);
    setIsLoading(true);
    setFeedbackMessage({type: 'info', text: 'Checking file and selections...'})

    if (!sourceType || !accountSource || !objectIdSource) {
      setFeedbackMessage({ type: 'error', text: 'Please ensure all selections (Platform, Account Source, Object ID Source) are made.' });
      setIsLoading(false);
      setFileName('');
    ...
    Papa.parse(file, {
      ...
      skipEmptyLines: true,
      complete: (results) => {
        setTimeout(() => { // Allow UI update for loading state
            try {
              setIsLoading(true); // Ensure it's set if somehow reset
              setFeedbackMessage({type: 'info', text: 'Processing rows... This may take a moment for large files.'})

              let skipped = 0;
              const headers = results.meta?.fields;

              if (!headers || results.data.length === 0) {
                throw new Error('The CSV file appears to be empty or does not contain header rows.');
              }
              ...
              if (missingHeaders.length > 0) {
                  throw new Error(`Missing required columns in CSV: ${missingHeaders.join(', ')}. Check selections.`);
              }

              // --- Row Transformation & Validation ---
               const transformed: CSDSRow[] = results.data
                .map((row: any, index: number): CSDSRow | null => {
                    // Simplified try/catch for row processing
                    try {
                        ...
                        // --- Field Extraction (Based on sourceType) ---
                         if (sourceType === 'telegram') {
                            contentIdVal = row.message_id; timestampVal = row.date; objectIdSourceVal = row.message_text;
                            if (accountSource === 'telegram_channel') { accountIdVal = row.channel_id; accountName = row.channel_name;}
                            else { accountIdVal = row.sender_id; accountName = row.post_author; }
                        } else if (sourceType === 'youtube') {
                            accountIdVal = row.channelId; accountName = row.channelTitle; contentIdVal = row.videoId; timestampVal = row.publishedAt; objectIdSourceVal = row[objectIdSource!];
                        } else if (sourceType === 'bluesky') {
                            accountIdVal = row.username; accountName = row.username; contentIdVal = row.id; timestampVal = row.date; objectIdSourceVal = row.text;
                        } else if (sourceType === 'tiktok') {
                            accountIdVal = row.author_name; accountName = row.author_name; contentIdVal = row.video_id; timestampVal = row.create_time; objectIdSourceVal = row[objectIdSource!];
                            ...
                            accountIdVal = row[idField]; accountName = row[nameField]; contentIdVal = row.id; timestampVal = row.creation_time;
                            objectIdSourceVal = (objectIdSource === 'text') ? row.text : (sourceType === 'facebook' ? row['link_attachment.link'] : undefined);
                        }

                        // --- Basic Validation ---
                        if (!accountIdVal || !contentIdVal || !timestampVal || (objectIdSourceVal === undefined)) {
                             throw new Error("Missing core identifier fields (account, content, timestamp, or object source value)");
                        }
                         // Allow objectIdSourceVal to be null/empty string unless specific checks added later


                        // --- Timestamp Parsing ---
                        let finalTimestamp = 0;
                         if (typeof timestampVal === 'number') {
                            finalTimestamp = (timestampVal > 100000000000) ? Math.floor(timestampVal / 1000) : timestampVal;
                        } else if (typeof timestampVal === 'string') {
                           const date = new Date(timestampVal);
                           if (!isNaN(date.getTime())) {
                               finalTimestamp = Math.floor(date.getTime() / 1000);
                           } else {
                               const numVal = Number(timestampVal); // Try parsing stringified number
                               if (!isNaN(numVal) && numVal > 0) {
                                   finalTimestamp = (numVal > 100000000000) ? Math.floor(numVal / 1000) : numVal;
                               } else { throw new Error('Invalid date string'); }
                           }
                        } else { throw new Error('Invalid timestamp type'); }

                        if (finalTimestamp <= 0) throw new Error('Invalid timestamp value');
                        ...
                        const safeAccountIdVal = String(accountIdVal || '').trim();
                         ...
                         if (sourceType === 'telegram' || sourceType === 'youtube' || sourceType === 'facebook' || sourceType === 'instagram') {
                             if (!safeAccountName || !safeAccountIdVal) throw new Error("Missing name/ID for account");
                             finalAccountId = `${safeAccountName} (${safeAccountIdVal})`;
                        } else if (sourceType === 'bluesky') {
                             finalAccountId = safeAccountIdVal;
                        ...
                        } else { throw new Error("Unhandled source type"); }


                        // --- Object ID Construction ---
                        const finalObjectId = (objectIdSourceVal !== undefined && objectIdSourceVal !== null) ? String(objectIdSourceVal) : '';


                        // --- Content ID Construction ---
                        const finalContentId = String(contentIdVal).trim();
                        if (!finalContentId) throw new Error("Missing content ID");
              ...
              if (transformed.length === 0) {
                throw new Error(`No valid data rows processed. ${skipped > 0 ? `(${skipped} rows skipped).` : ''} Check file content and required columns.`);
              }

              // Calculate size and set state for strategy options
              ...
              if (totalSizeMB > CSDS_SIZE_LIMIT_MB) {
                ...
                const avgRowSize = totalSize / transformed.length || 1; // Avoid division by zero
                const calculatedSampleSize = Math.max(1, Math.floor(TARGET_CHUNK_SIZE_BYTES / avgRowSize));
                ...
                setFeedbackMessage({
                    type: 'warning',
                    text: `Output file is ${totalSizeMB.toFixed(1)}MB, exceeding the CSDS ${CSDS_SIZE_LIMIT_MB}MB limit. Choose a strategy below.`
                });
              } else {
                setLargeFileStrategy(null);
                setFeedbackMessage({
                    type: 'success',
                    text: `Processing complete. File size is ${totalSizeMB.toFixed(1)}MB. Ready for download.`
                });
              }

            } catch (err) {
              console.error('Overall processing error:', err);
              // Reset data state on error to hide download buttons etc.
              setTransformedData(null);
              setEstimatedSizeMB(0);
  ...
  const handleLargeFileStrategyChange = (strategy: LargeFileStrategy) => {
  ...
  const handleSampleSizeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const maxRows = transformedData?.length || suggestedSampleSize || 1;
  ...
  const getSampledData = useCallback((): CSDSRow[] | null => {
    ...
    switch (sampleType) {
      ...
      case 'chrono':
        ...
        return dataToSample.slice(0, finalSampleSize);
      case 'even':
        const step = dataToSample.length / finalSampleSize; // Can be float
        const sampled: CSDSRow[] = [];
        const indices = new Set<number>();
         for (let i = 0; i < finalSampleSize; i++) {
            const index = Math.floor(i * step);
            if (index >= dataToSample.length) break; // Prevent out of bounds
            if (!indices.has(index)) { // Avoid duplicates for small steps/large samples
                sampled.push(dataToSample[index]);
                indices.add(index);
            }
        }
         // Ensure we return exactly finalSampleSize if possible, handle edge cases
        while (sampled.length < finalSampleSize && indices.size < dataToSample.length) {
            // If we didn't get enough due to flooring/duplicates, try adding adjacent unique elements
            let added = false;
            for(let i = 0; i < dataToSample.length; i++) {
                if (!indices.has(i)) {
                    sampled.push(dataToSample[i]);
                    indices.add(i);
                    added = true;
                    break;
                }
            }
            if (!added) break; // Safety break if all elements are somehow used
        }
        // Final trim if oversampled due to edge cases
        return sampled.slice(0, finalSampleSize);
      default: return null;
    }
  }, [transformedData, sampleType, sampleSize]); // Dependencies for useCallback

   // Generic download utility
   const downloadData = useCallback((getData: () => CSDSRow[] | null, baseFilename: string, suffix: string, statusUpdate: string) => {
    if (isLoading) return; // Prevent multiple concurrent downloads
    ...
    setTimeout(() => {
        try {
            ...
            setFeedbackMessage({ type: 'success', text: `Download initiated: ${downloadFilename}. Check browser downloads.` });
        } catch (error) {
  ...
  }, [isLoading, transformedData]); // Dependencies for useCallback
  ...
  }, [sourceType, sampleType, sampleSize, getSampledData, downloadData]); // Include dependencies


  const handleDownloadSplitUser = useCallback(() => {
    if (!transformedData || splitParts < minSplitParts || isLoading) {
        if (!isLoading) setFeedbackMessage({ type: 'error', text: `Invalid number of parts. Minimum is ${minSplitParts}.` });
        return;
    }
    ...
    const downloadChunksSequentially = async () => {
        try {
            ...
            if (transformedData.length < 1) throw new Error("Cannot split file with less than one row of data."); // Handle edge case

            // Safely get header using the first row
            ...
            setFeedbackMessage({ type: 'success', text: `Initiated download for ${downloadsInitiated} split files. Check browser downloads.` });
        } catch (error) {
     ...
     // Use setTimeout to ensure UI updates before async function starts
     setTimeout(downloadChunksSequentially, 50);

  }, [transformedData, splitParts, minSplitParts, isLoading, sourceType]);


  const handleDownloadProceedAnyway = useCallback(() => {
  ...
  }, [sourceType, estimatedSizeMB, transformedData, downloadData]);
  ...
  const handleFinalDownload = useCallback(() => {
    ...
    if (transformedData && estimatedSizeMB <= CSDS_SIZE_LIMIT_MB) {
  ...
  }, [isLoading, transformedData, estimatedSizeMB, largeFileStrategy, handleDownloadSingle, handleDownloadSample, handleDownloadSplitUser, handleDownloadProceedAnyway]);


  const isDownloadStrategyReady = useCallback((): boolean => {
    if (isLoading) return false;
    if (!transformedData) return false;

    if (estimatedSizeMB <= CSDS_SIZE_LIMIT_MB) return true;
  ...
  }, [isLoading, transformedData, estimatedSizeMB, largeFileStrategy, sampleType, sampleSize, splitParts, minSplitParts]);
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div className="p-4 sm:p-8 space-y-5 sm:space-y-6">
          ...
          {/* --- Step 1: Source Selection --- */}
           <div className={getStepClasses(true)}>
                  ...
                  <div className="space-y-3">
                     ...
                     {isLoading && (
                         <div className="flex items-center justify-center text-gray-600 pt-2">
                             ...
                             {feedbackMessage?.type === 'info' ? feedbackMessage.text : 'Processing...'} please wait.
                         </div>
                     )}
                     {/* Required Columns Info */}
                     {step4Enabled && !isLoading && !transformedData && (
                        <Alert type="info">
                            <span className="font-medium">Required Columns:</span> Ensure your CSV includes columns for your selections. E.g., for {sourceType || 'platform'} / {accountSource || 'account'} / {objectIdSource || 'object'}: {getRequiredHeaders().map(h => <code key={h} className="text-xs bg-blue-100 p-0.5 rounded mx-0.5">{h || 'N/A'}</code>)}
                        </Alert>
                     )}
                  </div>
              )}
          ...
          {transformedData && !isLoading && (
            <div className="mt-4 sm:mt-6 space-y-5 sm:space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
                    {(['sample', 'split', 'proceed'] as const).map(strategy => (
                      <label key={strategy} className={`flex items-center p-3 border rounded-lg transition-colors duration-200 ${isLoading ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'} ${largeFileStrategy === strategy ? 'border-yellow-600 bg-yellow-100 ring-2 ring-yellow-500' : 'border-yellow-300 hover:border-yellow-500 hover:bg-yellow-100'}`}>
                        <input type="radio" name="largeFileStrategy" value={strategy} checked={largeFileStrategy === strategy} onChange={() => handleLargeFileStrategyChange(strategy)} className="mr-3 h-4 w-4 text-yellow-700 focus:ring-yellow-600 border-yellow-400" disabled={isLoading}/>
                        <span className="font-medium text-yellow-900 text-sm sm:text-base">{strategy === 'sample' ? 'Create Sample' : strategy === 'split' ? 'Split File' : 'Download Full File'}</span>
                      </label>
                    ))}
                  </div>
                    ...
                    <div className="pl-4 sm:pl-6 border-l-4 border-yellow-300 ml-1 py-3 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-x-4 sm:gap-x-6 gap-y-2">
                         {(['random', 'chrono', 'even'] as const).map(type => (
                             <label key={type} className={`flex items-center text-sm ${isLoading || largeFileStrategy !== 'sample' ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'}`}>
                                 <input type="radio" name="sampleType" value={type} checked={sampleType === type} onChange={() => handleSampleTypeChange(type)} className="mr-2 h-4 w-4 text-yellow-700 focus:ring-yellow-600 border-yellow-400" disabled={isLoading || largeFileStrategy !== 'sample'}/>
                       ...
                       <div className="flex flex-wrap items-center gap-2">
                           ...
                           <input type="number" id="sample-size" value={sampleSize} onChange={handleSampleSizeChange} min="1" max={transformedData.length} className={`block w-full max-w-[120px] sm:max-w-[150px] rounded-md border-yellow-400 shadow-sm focus:border-yellow-600 focus:ring-yellow-600 sm:text-sm p-1.5 sm:p-2 ${isLoading || largeFileStrategy !== 'sample' || !sampleType ? 'bg-gray-100 cursor-not-allowed' : ''}`} disabled={isLoading || largeFileStrategy !== 'sample' || !sampleType} aria-describedby="sample-size-hint"/>
                  ...
                  {largeFileStrategy === 'split' && (
                     <div className="pl-4 sm:pl-6 border-l-4 border-yellow-300 ml-1 py-3 space-y-3">
                       ...
                       <div className="flex flex-wrap items-center gap-2">
                           ...
                           <input type="number" id="split-parts" value={splitParts} onChange={handleSplitPartsChange} min={minSplitParts} className={`block w-full max-w-[80px] sm:max-w-[100px] rounded-md border-yellow-400 shadow-sm focus:border-yellow-600 focus:ring-yellow-600 sm:text-sm p-1.5 sm:p-2 ${isLoading || largeFileStrategy !== 'split' ? 'bg-gray-100 cursor-not-allowed' : ''}`} disabled={isLoading || largeFileStrategy !== 'split'} aria-describedby="split-parts-hint"/>
              ...
              {(transformedData && (estimatedSizeMB <= CSDS_SIZE_LIMIT_MB || largeFileStrategy)) && (
                ...
                >
                  {/* Icon and Text logic */}
                  {isLoading ? ( /* Show spinner if ANY download/processing is happening */
           ...
           <div className="bg-gray-50 p-3 sm:p-4 text-center text-xs text-gray-500 border-t border-gray-200 mt-4 sm:mt-6">
             <p>CSDS Pre-processor v1.4.1 - Large File Strategies</p>
             <p className="mt-1">
