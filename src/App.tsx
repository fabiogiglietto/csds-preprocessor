...
function App() {
  ...
  // Add these functions for the FileSizeModal component
  const handleSplitDownload = () => {
    if (!transformedData) return;
    
    const zip = new JSZip();
    const maxRowsPerFile = Math.ceil(transformedData.length / Math.ceil(estimatedFileSizeMB / 14));
    ...
    zip.generateAsync({type:"blob"}).then(function(content) {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `${sourceType}_csds_split_${new Date().toISOString().slice(0,10)}.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      ...
      setFeedbackMessage({ 
        type: 'success', 
        text: `Successfully split data into multiple files and created a ZIP archive for download.` 
      });
    });
  };
  
  const handleTimeBasedSplit = (periodDays: number) => {
    if (!transformedData) return;
    
    // Group data by time periods
    const periodGroups = new Map<string, CSDSRow[]>();
    
    transformedData.forEach(row => {
      const date = new Date(row.timestamp_share * 1000);
      const periodKey = `${date.getFullYear()}-${Math.floor(date.getMonth() / (12/Math.round(365/periodDays)))}`;
    ...
    zip.generateAsync({type:"blob"}).then(function(content) {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `${sourceType}_csds_by_period_${new Date().toISOString().slice(0,10)}.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      ...
      setFeedbackMessage({ 
        type: 'success', 
        text: `Successfully split data into ${periodGroups.size} time periods and created a ZIP archive for download.` 
      });
    });
  };
  
  const handleStratifiedSampling = (percentage: number) => {
    if (!transformedData) return;
    
    // Group by account_id for stratified sampling
    const accountGroups = new Map<string, CSDSRow[]>();
    ...
    // Create sample that maintains account distribution
    const sampledData: CSDSRow[] = [];
    
    Array.from(accountGroups.entries()).forEach(([_, rows]) => {
      const sampleSize = Math.max(1, Math.round(rows.length * percentage / 100));
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', `${sourceType}_csds_sample_${percentage}pct_${new Date().toISOString().slice(0,10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
